...
class WsListener {
  ...
  constructor(options) {
    ...
    this.wss.forEach((wss, path) => {
      handleConnections(self, wss, path);
    });
  }
}
...
function handleConnections(instance, wss, path) {
  wss.on("connection", function (ws) {
    ...
    function handleSbMessage(message) {
      ws.send(message);
    }
    let decodingOptions = Object.assign(
      { responseHandler: handleSbMessage },
      instance.decodingOptions
    );

    ws.on("message", (message) => {
      console.log("Aruba WebSocket message received on", path);
      console.log(message);
      let time = new Date().getTime();

      instance.decoder.handleProtobuf(message, path, time, decodingOptions);
    });
  });
}
