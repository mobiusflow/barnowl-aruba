...
class ArubaDecoder {
  ...
  handleProtobuf(data, origin, time, decodingOptions) {
    ...
    if (typeof origin === "string" && origin.includes("mobiusflow")) {
      try {
        let nbData =
          aos10Proto.aruba.iotops.partner.transport.v1.TransportNBData.decode(
            data
          );
        nbData.toJSON();

        console.log("AOS10 NB data received");
        console.log(`topic: ${nbData.header.topic}`);
        // console.log(JSON.stringify(nbData, null, 2));
        // console.log(`Origin: ${origin}`);

        // TODO: accept raddecs and infrastructureMessages
        raddecs = iotOpsDecoder.decode(
          nbData,
          origin,
          time,
          self.aps,
    ...
    // Otherwise assume AOS8
      ...
      try {
        ...
        let decoded = telemetryReportDecoder.decode(
          telemetryReport,
          origin,
          time,
          self.aps,
  ...
  handleData(data, origin, time, decodingOptions) {
    ...
    let raddecs = telemetryReportDecoder.decode(
      data,
      origin,
      time,
      self.aps,
    ...
    raddecs.raddecs.forEach(function (raddec) {
      self.barnowl.handleRaddec(raddec);
    });
  }
}
